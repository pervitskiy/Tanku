...
public class Bullet {
    ...
    public Bullet(float x , float y , float scale , float speed , String direction , TextureAtlas atlas , Level lvl ,
        ...
        explosionList
                .add(new Sprite(
                        new SpriteSheet(atlas.cut(16 * Player.SPRITE_SCALE , 8 * Player.SPRITE_SCALE ,
                                Player.SPRITE_SCALE , Player.SPRITE_SCALE) , Entity.SPRITES_PER_HEADING , Player.SPRITE_SCALE) ,
                        scale));
        explosionList
                .add(new Sprite(
        ...
        for (BulletHeading bh : BulletHeading.values()) {
            SpriteSheet sheet = new SpriteSheet(bh.texture(atlas) , Entity.SPRITES_PER_HEADING , Player.SPRITE_SCALE / 2);
    ...
    private boolean isDestroyableTile(int tileNum) {
        if (tileNum == TileType.BRICK.numeric()
                || tileNum == TileType.DOWN_LEFT_EAGLE.numeric()
                || tileNum == TileType.DOWN_RIGHT_EAGLE.numeric()
                || tileNum == TileType.UP_LEFT_EAGLE.numeric()
                || tileNum == TileType.UP_RIGHT_EAGLE.numeric()
                || tileNum == TileType.METAL.numeric() && type == EntityType.Player && Player.getPlayerStrength() == 3)//тип имбовый уровень, ломающий меттал {
            return true;
        return false;
    }
    ...
    private boolean isImpassableTile(Integer... tileNum) {
        for (int i = 0; i < tileNum.length; i++) {
            if (tileNum[i] == TileType.BRICK.numeric()
                    || tileNum[i] == TileType.METAL.numeric()
                    || tileNum[i] == TileType.DOWN_LEFT_EAGLE.numeric()
                    || tileNum[i] == TileType.DOWN_RIGHT_EAGLE.numeric()
                    || tileNum[i] == TileType.UP_LEFT_EAGLE.numeric()
                    || tileNum[i] == TileType.UP_RIGHT_EAGLE.numeric()
    ...
    public void drawExplosion(Graphics2D g) {
        ...
        if (animationCount % 15 < 5)
            explosionList.get(0).render(g , adjustedX , adjustedY);
        else if (animationCount % 15 >= 5 && animationCount % 15 < 10)
            explosionList.get(1).render(g , adjustedX , adjustedY);
        else if (animationCount % 15 > 12)
    ...
    public enum BulletHeading {
        ...
        B_SOUTH(21 * Player.SPRITE_SCALE , 6 * Player.SPRITE_SCALE + 4 , Player.SPRITE_SCALE / 2 ,
                1 * Player.SPRITE_SCALE / 2),
        B_WEST(20 * Player.SPRITE_SCALE + Player.SPRITE_SCALE / 2 , 6 * Player.SPRITE_SCALE + 4 , Player.SPRITE_SCALE / 2 ,
                1 * Player.SPRITE_SCALE / 2);
